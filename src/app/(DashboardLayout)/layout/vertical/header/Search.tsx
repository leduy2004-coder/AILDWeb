...
const Search = () => {
  ...
  const filterRoutes = (rotr: AnyType, cSearch: string) => {
    if (rotr.length > 1)
      return rotr.filter((t: AnyType) =>
        t.title
  ...
  return (
    <>
      ...
      <Button
        ...
        onClick={handleClick}
        startIcon={
          <Icon color="#ffffff" icon="solar:magnifer-linear" width={20} />
        }
        sx={{
      ...
      >
        <Box sx={{ width: '150px', color: '#ffffff' }}>
          {t('button.search')}
        </Box>
      </Button>
      <Menu
